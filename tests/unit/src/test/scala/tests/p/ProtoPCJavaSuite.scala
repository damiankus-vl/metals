...
class ProtoPCJavaSuite extends BaseProtoPCSuite("proto-pc-java") {
  ...
  test("java-navigates-within-generated-outline-outer-class") {
    ...
    for {
      ...
      // type Authentication resolves within the same file, and the proto it
      // was generated from is offered alongside it.
  ...
  // A nested message may repeat a field name from its parent. Matching the owner
  // chain, not a single class name, is what keeps the two apart at either depth.
  test("java-navigates-to-shadowed-nested-field") {
    cleanWorkspace()
    for {
      _ <- initialize(
        """|/metals.json
           |{"a": {}}
           |/a/src/main/proto/container.proto
           |syntax = "proto3";
           |package com.example.api;
           |option java_package = "com.example.api.jproto";
           |option java_multiple_files = true;
           ...
           |import com.example.api.jproto.Container;
           |public class Client {
           |  public String outer(Container c) { return c.getValue(); }
           |  public String inner(Container.Inner i) { return i.getValue(); }
           |}
           |""".stripMargin
      )
      _ <- server.didOpen("a/src/main/proto/container.proto")
      _ <- server.didOpen("a/src/main/java/com/example/Client.java")
      _ <- server.didFocus("a/src/main/java/com/example/Client.java")
      _ = assertNoDiagnostics()
      _ <- assertProtoDefinition(
        "a/src/main/java/com/example/Client.java",
        "return c.getVa@@lue();",
        """|a/src/main/proto/container.proto:6:10: definition
           |  string value = 1;
           |         ^^^^^
           |""".stripMargin,
      )
      _ <- assertProtoDefinition(
        "a/src/main/java/com/example/Client.java",
        "return i.getVa@@lue();",
        """|a/src/main/proto/container.proto:8:12: definition
           |    string value = 1;
           |           ^^^^^
           |""".stripMargin,
      )
  ...
  // A generator may have already written the Java Metals would otherwise only
  // synthesize, under the very same fully qualified name.
  test("java-third-party-generated-file-with-outline-name") {
    cleanWorkspace()
    for {
      _ <- initialize(
        """|/metals.json
           ...
           |  string token = 1;
           |}
           |/a/src/main/java/com/example/api/jproto/ClientProtos.java
           |// Generated by the protocol buffer compiler.  DO NOT EDIT!
           |// source: a/src/main/proto/client.proto
           |package com.example.api.jproto;
           |public final class ClientProtos {
           |  private ClientProtos() {}
           |  public static final class Session {
           |    public String getToken() { return ""; }
           |    public String tokenAsString() { return ""; }
           |  }
           |}
           |/a/src/main/java/com/example/Client.java
           |package com.example;
           |import com.example.api.jproto.ClientProtos;
           |public class Client {
           |  public String open(ClientProtos.Session session) {
      ...
      _ <- server.didFocus("a/src/main/java/com/example/Client.java")
      _ = assertNoDiagnostics()
      // The generator's own file wins. The outline declares the same class, so
      // offering it too would only add a stub copy of an entry already listed;
      // the proto is the one extra worth having.
      _ <- assertDefinitionFileOrder(
        "a/src/main/java/com/example/Client.java",
        "public String open(ClientProtos.Sess@@ion session)",
        List(
          "a/src/main/java/com/example/api/jproto/ClientProtos.java",
          "a/src/main/proto/client.proto",
        ),
      )
      // An accessor the generator wrote resolves the same way.
      _ <- assertProtoDefinition(
        "a/src/main/java/com/example/Client.java",
        "return session.getTo@@ken();",
        """|a/src/main/proto/client.proto:6:10: definition
           |  string token = 1;
           |         ^^^^^
           |""".stripMargin,
      )
      // Type-checking has to use the generator's file, not the outline: both
      // answer the same name, and javac picks by modification time -- which a
      // just-materialized outline always wins.
      _ <- server.didChange("a/src/main/java/com/example/Client.java") { _ =>
        """|package com.example;
           |import com.example.api.jproto.ClientProtos;
           |public class Client {
           |  public String open(ClientProtos.Session session) {
      ...
      // Declared only by the generator's file, so its presence means that file
      // is what the name resolved to.
      _ = assert(
        completions.contains("tokenAsString"),
        s"expected tokenAsString in completions:\n$completions",
      )
      // Declared only by the outline, so its absence means the outline is not
      // standing in for a file that is already there.
      _ = assert(
        !completions.contains("getTokenBytes"),
        s"unexpected outline member getTokenBytes in completions:\n$completions",
      )
    } yield ()
  }

  // One `.proto` produces several files that all declare the queried type, so all
  // of them belong in the dropdown. `java_multiple_files` is where this bites:
  // grpc-java nests `ImplBase` inside `<Svc>Grpc` while plenty of Java imports it
  // as a top-level class, and both spellings are the same declaration.
  test("java-multiple-files-offers-every-matching-output") {
    cleanWorkspace()
    for {
      _ <- initialize(
        """|/metals.json
           ...
           |}
           |/a/src/main/java/com/example/api/jproto/GreeterGrpc.java
           |// Generated by the protocol buffer compiler.  DO NOT EDIT!
           |// source: a/src/main/proto/greeter.proto
           |package com.example.api.jproto;
           ...
           |  }
           |}
           |/a/src/main/java/com/example/api/jproto/GreeterImplBase.java
           |// Generated by the protocol buffer compiler.  DO NOT EDIT!
           |// source: a/src/main/proto/greeter.proto
           |package com.example.api.jproto;
           |public abstract class GreeterImplBase extends GreeterGrpc.GreeterImplBase {
           |}
      ...
      _ <- server.didFocus("a/src/main/java/com/example/GreeterImpl.java")
      // Every output declaring the type, then the proto. The top-level alias goes
      // first because it declares the symbol exactly as asked for; the nested
      // spelling in `GreeterGrpc.java` is the same declaration reached through its
      // owner chain, and dropping it would hide a real answer.
      _ <- assertDefinitionFileOrder(
        "a/src/main/java/com/example/GreeterImpl.java",
        "public class GreeterImpl extends GreeterImpl@@Base {",
        List(
          "a/src/main/java/com/example/api/jproto/GreeterImplBase.java",
          "a/src/main/java/com/example/api/jproto/GreeterGrpc.java",
          "a/src/main/proto/greeter.proto",
      ...
      // The proto entry lands on the service, not on the file or a sibling rpc.
      _ <- assertProtoDefinition(
        "a/src/main/java/com/example/GreeterImpl.java",
        "public class GreeterImpl extends GreeterImpl@@Base {",
        """|a/src/main/proto/greeter.proto:8:9: definition
           |service Greeter {
           |        ^^^^^^^
           |""".stripMargin,
      )
