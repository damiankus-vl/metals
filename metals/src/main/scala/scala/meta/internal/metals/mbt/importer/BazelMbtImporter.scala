...
abstract class BazelMbtImporter(
  ...
  /**
   * Rules whose compiled class jar is worth recording in `classDirectories`:
   * runnable targets plus plain libraries. Library targets were previously
   * omitted, so navigation couldn't reach members that exist only in compiled
   * output (e.g. Lombok-generated accessors, which the header compiler never
   * produces).
   */
